    ...
    interface rot
        ...
        module procedure q_rotation_matrix, q_rotate_vector
    ...
    pure function q_rotate_vector(q, v) result(u)
    type(quaternion), intent(in) :: q
    type(vector3), intent(in) :: v
    type(vector3) :: u
    real(wp) :: x,y,z,w,vx,vy,vz,xx,yy,zz
    x = q%vector%x
    y = q%vector%y
    z = q%vector%z
    w = q%scalar
    ...
    u%x =  2*x*(vy*y+vz*z)-2*vx*yy+2*vz*w*y-2*vx*zz-2*vy*w*z+vx
    u%y = -2*vy*xx+2*x*(vx*y-vz*w)+2*vz*y*z-2*vy*zz+2*vx*w*z+vy
    u%z = -2*vz*xx+2*x*(vx*z+vy*w)-2*vz*yy+2*y*(vy*z-vx*w)+vz
    
    end function
    
    pure function q_rotation_matrix(q, inverse) result(R)
